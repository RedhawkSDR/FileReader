...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.FileReader.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.FileReader-4.0.2
        tar czf ${tmpdir}/rh.FileReader-4.0.2.tar.gz --exclude=".svn" -C ${tmpdir} rh.FileReader-4.0.2
        rpmbuild -ta ${tmpdir}/rh.FileReader-4.0.2.tar.gz
        rm -rf $tmpdir
    else
